...
function LockIcon() {
    return (
        <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--text-muted)" }}>
...
/* ─── Helper: fetch + decrypt attachments from Storage ───────── */
async function loadAttachmentsForNote(
    noteId: string,
    noteKey: CryptoKey,
): Promise<DecryptedAttachment[]> {
    try {
        const res = await fetch(`/api/notes/attachments/${noteId}`);
        if (!res.ok) return [];
        ...
        return await Promise.all(
            attachments.map(async (a: any) => {
                ...
                const dlRes = await fetch(
                    `/api/notes/attachments/download/${encodeURIComponent(a.storage_path)}`
                );
                const encryptedBuffer = await dlRes.arrayBuffer();

                // Decrypt raw bytes
                const decryptedBuffer = await decryptBuffer(noteKey, encryptedBuffer, a.iv);

                // Create in-memory blob URL
                const blobUrl = URL.createObjectURL(
                    new Blob([decryptedBuffer], { type: a.mime_type })
                );
        ...
        console.error("Failed to load attachments for note", noteId, err);
        return [];
    }
}
...
function NoteEditor({
        ...
        const timer = setTimeout(async () => {
            try {
                ...
                await fetch("/api/notes/update", {
                    ...
                    body: JSON.stringify({ id: activeNote.id, content_cipher: cipher, content_iv: iv }),
                });
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            try {
                const { cipher: title_cipher, iv: title_iv } = await encrypt(activeNote.noteKey, title);
                await fetch("/api/notes/update", {
                    method: "PATCH",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ id: activeNote.id, title_cipher, title_iv }),
                });
    ...
    return (
        <main style={{ flex: 1, display: "flex", flexDirection: "column", padding: 0, overflow: "hidden" }}>
            {/* Title bar */}
            ...
            {/* Image dock — handles its own upload/delete */}
            <ImageDock
                ...
                onAttachmentRemoved={(attachmentId) =>
                    updateNote(activeNote.id, {
                        attachments: (activeNote.attachments ?? []).filter(
                            (a) => a.id !== attachmentId
                        ),
                    })
                }
...
export default function DashboardPage() {
    ...
    const { notes, activeNoteId, setNotes, addNote, deleteNote, setActiveNoteId } = useNotesStore();
    ...
    const { sharedNotes, activeSharedNoteId, setSharedNotes, setActiveSharedNoteId, clearSharedNotes } = useSharedNotesStore();
    ...
    }, [activeTab, derivedKey, username]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        try {
            ...
            const decrypted: DecryptedNote[] = await Promise.all(
                encryptedNotes.map(async (note: any) => {
                    ...
                    // Fetch + decrypt attachments from Storage
                    const attachments = await loadAttachmentsForNote(note.id, noteKey);
    ...
    async function loadSharedNotes() {
        ...
        try {
            ...
            if (!userRes.ok) throw new Error("Failed to fetch your ECDH keys");
            ...
            const decryptedSharedNotes: SharedDecryptedNote[] = await Promise.all(
                encryptedSharedNotes
                    .filter((sharedNote: any) => sharedNote.note !== null)
                    .map(async (sharedNote: any) => {
                        ...
                        const title = await decrypt(noteKey, sharedNote.note.titleCipher, sharedNote.note.titleIv);

                        // Fetch + decrypt attachments from Storage using the note's id
                        const attachments = await loadAttachmentsForNote(sharedNote.noteId, noteKey);
    ...
    async function createNote() {
        ...
        try {
            ...
            const { cipher: title_cipher, iv: title_iv } = await encrypt(noteKey, "Untitled");
            const { cipher: content_cipher, iv: content_iv } = await encrypt(noteKey, "");
            const { cipher: wrapped_key_cipher, iv: wrapped_key_iv } = await wrapNoteKey(noteKey, derivedKey);

            const res = await fetch("/api/notes/create", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ title_cipher, title_iv, content_cipher, content_iv, wrapped_key_cipher, wrapped_key_iv }),
            });
        ...
        <>
            <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
                ...
                <header className="topbar">
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                        <div className="font-mono" style={{ fontSize: 12, color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e", boxShadow: "0 0 6px #22c55e", display: "inline-block" }} />
                            {username}
                        </div>
                        <button id="signout-btn" onClick={handleSignOut} className="btn btn-ghost" style={{ fontSize: 12, padding: "7px 14px" }}>
                            Sign Out
                        </button>
                    </div>
                </header>
                ...
                <div style={{ display: "flex", flex: 1 }}>
                    ...
                    <aside className="sidebar">
                        <div style={{ display: "flex", gap: 8, marginBottom: 16, borderBottom: "1px solid var(--border)", paddingBottom: 12 }}>
                            <button onClick={() => setActiveTab("my")} style={{ flex: 1, background: activeTab === "my" ? "rgba(230, 57, 70, 0.1)" : "transparent", border: activeTab === "my" ? "1px solid rgba(230, 57, 70, 0.3)" : "1px solid var(--border)", borderRadius: 6, padding: "8px 12px", fontSize: 12, fontWeight: 500, color: activeTab === "my" ? "var(--accent)" : "var(--text-secondary)", cursor: "pointer", fontFamily: "monospace" }}>
                                My Notes
                            </button>
                            <button onClick={() => setActiveTab("shared")} style={{ flex: 1, background: activeTab === "shared" ? "rgba(230, 57, 70, 0.1)" : "transparent", border: activeTab === "shared" ? "1px solid rgba(230, 57, 70, 0.3)" : "1px solid var(--border)", borderRadius: 6, padding: "8px 12px", fontSize: 12, fontWeight: 500, color: activeTab === "shared" ? "var(--accent)" : "var(--text-secondary)", cursor: "pointer", fontFamily: "monospace" }}>
                        ...
                        {activeTab === "my" ? (
                            <>
                                <div className="font-mono" style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-muted)", padding: "4px 12px", marginBottom: 8 }}>
                                    My Notes
                                </div>
                                <Tooltip content="Create New Note" delayDuration={100}>
                                    <button id="new-note-btn" className="btn btn-secondary" style={{ width: "100%", justifyContent: "flex-start", gap: 10, fontSize: 13, padding: "10px 14px" }} onClick={createNote}>
                                        <PlusIcon /> New Note
                                    </button>
                                </Tooltip>
                                {loadingNotes ? (
                                    <div style={{ padding: "20px 12px", display: "flex", alignItems: "center", gap: 10 }}>
                                        <div className="spinner" />
                                        <span className="font-mono" style={{ fontSize: 11, color: "var(--text-muted)" }}>Decrypting…</span>
                                    </div>
                                ) : notes.length === 0 ? (
                                    <div style={{ padding: "20px 12px", textAlign: "center", fontSize: 12, color: "var(--text-muted)" }}>No notes yet. Create one!</div>
                                ) : (
                                    <ul style={{ listStyle: "none", marginTop: 4 }}>
                                        {notes.map((note) => (
                                            <li key={note.id} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                                <button className="btn btn-ghost" onClick={() => setActiveNoteId(note.id)} style={{ flex: 1, fontSize: 13, textAlign: "left", justifyContent: "flex-start", padding: "8px 12px", background: note.id === activeNoteId ? "rgba(230, 57, 70, 0.08)" : "transparent", color: note.id === activeNoteId ? "var(--accent)" : "var(--text-secondary)", borderRadius: 6, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                    {note.title || "Untitled"}
                                                </button>
                                                <Tooltip content="Share" delayDuration={200}>
                                                    <button className="btn btn-ghost" onClick={() => { setSelectedNoteForShare(note); setIsShareModalOpen(true); }} style={{ padding: "6px 8px", color: "var(--text-muted)", flexShrink: 0 }}>
                                                        <ShareIcon />
                                                    </button>
                                                </Tooltip>
                                                <Tooltip content="Delete" delayDuration={200}>
                                                    <button className="btn btn-ghost" onClick={() => handleDeleteNote(note.id)} style={{ padding: "6px 8px", color: "var(--text-muted)", flexShrink: 0 }}>
                                                        <TrashIcon />
                                                    </button>
                                                </Tooltip>
                                            </li>
                            ...
                            <>
                                ...
                                {loadingSharedNotes ? (
                                    <div style={{ padding: "20px 12px", display: "flex", alignItems: "center", gap: 10 }}>
                                        <div className="spinner" />
                                        <span className="font-mono" style={{ fontSize: 11, color: "var(--text-muted)" }}>Loading shared notes…</span>
                                    </div>
                                ) : sharedNotes.length === 0 ? (
                                    <div style={{ padding: "20px 12px", textAlign: "center", fontSize: 12, color: "var(--text-muted)" }}>No notes shared with you yet</div>
                                ) : (
                                    <ul style={{ listStyle: "none", marginTop: 4 }}>
                                        {sharedNotes.map((note) => (
                                            <li key={note.shareId} style={{ display: "flex", alignItems: "center", gap: 4 }}>
                                                <button className="btn btn-ghost" onClick={() => setActiveSharedNoteId(note.shareId)} style={{ flex: 1, fontSize: 13, textAlign: "left", justifyContent: "flex-start", padding: "8px 12px", background: note.shareId === activeSharedNoteId ? "rgba(230, 57, 70, 0.08)" : "transparent", color: note.shareId === activeSharedNoteId ? "var(--accent)" : "var(--text-secondary)", borderRadius: 6, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                    {note.title || "Untitled"}
                                                    <span style={{ marginLeft: 8, fontSize: 10, color: "var(--text-muted)" }}>from {note.senderUsername}</span>
                                                </button>
                                            </li>
                                        ))}
                                    </ul>
                        ...
                        activeNote && derivedKey ? (
                            ...
                            <main style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16, padding: 40 }}>
                                ...
                                {!loadingNotes && notes.length === 0 && (
                                    <button className="btn btn-primary" onClick={createNote} style={{ marginTop: 8 }}>Create Note</button>
                                )}
            ...
            {selectedNoteForShare && (
                <ShareModal
                    ...
                    onClose={() => { setIsShareModalOpen(false); setSelectedNoteForShare(null); }}
                    noteId={selectedNoteForShare.id}
                    noteTitle={selectedNoteForShare.title}
                    onShareSuccess={() => { console.log("Note shared successfully"); }}
                />
            )}
        </>
    );
