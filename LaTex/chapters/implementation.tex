...
The application uses four database tables, all storing only encrypted or cryptographic data:

\begin{description}
    \item[\texttt{users}:] Stores user credentials as cryptographic artifacts---PBKDF2 salt, encrypted sentinel values for authentication, and an ECDH keypair (public key in plaintext, private key encrypted with the derived key).

    \item[\texttt{notes}:] Stores encrypted note content. Each note has separate ciphertext fields for title and content, along with a wrapped (encrypted) note key. The note key is encrypted with the user's derived key and is unique per note.

    \item[\texttt{shared\_notes}:] Records note sharing relationships. Contains the note key re-wrapped with an ECDH-derived shared secret, ensuring that only the intended recipient can unwrap it.

    \item[\texttt{attachments}:] Stores metadata for file attachments. The actual file content is stored as encrypted blobs in Supabase Storage, with the encryption IV recorded in this table for decryption.
\end{description}
